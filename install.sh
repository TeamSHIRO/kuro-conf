...
# kuro-sign installer
# Usage: curl -fsSL https://raw.githubusercontent.com/TeamSHIRO/kuro-conf/main/install.sh | bash

REPO_URL="https://github.com/TeamSHIRO/kuro-conf.git"
INSTALL_DIR="$HOME/.local/bin"
BUILD_DIR="/tmp/kuro-conf-build"
...
if [[ -f "$(pwd)/CMakeLists.txt" ]] && grep -q "KURO_CONF" "$(pwd)/CMakeLists.txt" 2>/dev/null; then
    info "Source directory detected. Building in place..."
    BUILD_DIR="$(pwd)"
else
    info "Cloning kuro-conf into $BUILD_DIR..."
    rm -rf "$BUILD_DIR"
...
# ── Install ────────────────────────────────────────────────────────────────────
mkdir -p "$INSTALL_DIR"
cp "$BUILD_DIR/build/kuro-conf" "$INSTALL_DIR/kuro-conf"
chmod +x "$INSTALL_DIR/kuro-conf"

success "kuro-conf installed to $INSTALL_DIR/kuro-conf"
...
if ! echo "$PATH" | grep -q "$INSTALL_DIR"; then
    ...
    if [[ -n "$SHELL_RC" ]]; then
        ...
        if [[ "$REPLY" =~ ^[Yy]$ ]]; then
            echo "" >> "$SHELL_RC"
            echo "# Added by kuro-conf installer" >> "$SHELL_RC"
...
# ── Cleanup ───────────────────────────────────────────────────────────────────
if [[ "$BUILD_DIR" == "/tmp/kuro-conf-build" ]]; then
    rm -rf "$BUILD_DIR"
fi

echo ""
success "kuro-conf is ready! Try: kuro-conf help"
